...
namespace UserRegistration
{
    public class UserDetails
    {
        ...
        [RegularExpression("^[0-9]{2}\\s[6-9]{1}[0-9]{9}", ErrorMessage = "Number is not valid")]
        ...
        [RegularExpression("^^[a-zA-Z0-9+_-]+(?:\\.[a-zA-Z0-9_-]+)*@[a-zA-Z0-9]+(?:\\.[a-zA-Z]{2,}){1,2}$", ErrorMessage = "Email is not valid")]
        ...
        [RegularExpression("^(?=.*[0-9])" + "(?=.*[a-z])(?=.*[A-Z])" + "(?=.*[@#$%^&+=])" + "(?=\\S+$).{8,}$", ErrorMessage = "Password is not valid")]
